...
Point=namedtuple('Point','x, y, l')#(x_cord,y_cord,length ) for bird it is set manually to zero
                                   # while in case of pipe it is randomlt selected
...
class flappybird:
    ...
    def reset(self):
        ...
        self.bird=Point(self.w/4,self.h/2,0)
        self.reward=0
        self.score=0
        self.jump=False
        self.should_append=True
        len_for_lower_Y=kuch_bhi()
        self.pipe=[Point(500,0,kuch_bhi()),Point(500,480-len_for_lower_Y,len_for_lower_Y)]#first time wala init
    ...
    def is_collision(self,edge_x=0,edge_y=0):
        ...
        for i in range(len(self.pipe)):
            if i%2==0:
                ...
                if self.bird.x+edge_x+SIZE>=self.pipe[i].x and self.bird.y+SIZE-edge_y>self.pipe[i].y:
                    return True
                return False
            elif self.bird.y>=480:
                return True
    ...
    def draw_pipe(self):
        for p in range(len(self.pipe)):
            ...
            y=self.pipe[p].y
            x-=10
            self.pipe[p]=Point(x,y,self.pipe[p].l)
        if len(self.pipe)>8:
            return
        for P in self.pipe:
        ...
        if self.pipe[0].x<random.randint(150,200) :
            ...
            self.pipe.append(Point(x,u_y,kuch_bhi()))
            self.pipe.append(Point(x,480-len_for_lower_Y,len_for_lower_Y))
